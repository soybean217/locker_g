...
<script type="text/javascript">
    function checkDeleteIds(ids) {
        ...
        if (flag) {
            ...
            alert("<s:text name='content.fileapprove.deleteroleright.error'/>");
            return false;
        }
    }
    function deleteRoleRights(ids) {
        if (checkDeleteIds(ids)) {
        	document.input.action='deleteRoleRights.action';
    ...
    function addRoleRight() {
        document.input.action = "toSaveRoleRight.action";
        document.input.submit();
        return;
    }
  </script>
<body>
<s:form name="input" action="manageRoleRights.action" method="post">
	<table width="100%" border="0" cellpadding="0" cellspacing="10"
		class="NormalText">
		<tr>
			<td class="TipWord"><s:text name="content.mainPage.tree.roleright" /></td>
		</tr>
		<tr>
			...
			<table width="100%" border="1" cellspacing="0" cellpadding="3"
				...
				<tr>
					<td><s:submit value="%{getText('allpgjsp.add')}"
						cssClass="FixButtonStyle" onclick="addRoleRight();" /> <input
						type="button" value='<s:text name="allpgjsp.del"/>'
						class="FixButtonStyle"
						onclick="deleteRoleRights(document.input.ids);" /></td>
				</tr>
			</table>
			</td>
		</tr>
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="0">
				...
				<THEAD>
					<tr>
						...
						<TH vAlign=center align=middle noWrap width=10%><s:text
							name='title.role' /></TH>
						<TH vAlign=center align=middle noWrap width=20%><s:text
                            name='cfgldpcrtjsp.desp' /></TH>
						<TH vAlign=center align=middle noWrap width=60%><s:text
							name='title.roleright' /></TH>
					</tr>
				</THEAD>

				<!--Resource View Table Entry -->
				<s:iterator value="roleRightBeans" status="brlist" id="roleRightBean">
					<tr
						...
						<td
							class="<s:if test='#brlist.Even'>DarkLogLine</s:if><s:else>LightLogLine</s:else>"
							width="5%">&nbsp;<a
							href="toSaveRoleRight.action?id=<s:property value='id'/>"><s:property
							value="(#request.cp - 1) * #request.pn + #brlist.index + 1" /></a></td>
						<td
							class="<s:if test='#brlist.Even'>DarkLogLine</s:if><s:else>LightLogLine</s:else>"
							width="10%">&nbsp;<s:property value="roleName" /></td>
					    <td
                            class="<s:if test='#brlist.Even'>DarkLogLine</s:if><s:else>LightLogLine</s:else>"
                            width="20%">&nbsp;<s:property value="description" /></td>
						<td
							class="<s:if test='#brlist.Even'>DarkLogLine</s:if><s:else>LightLogLine</s:else>"
							style="BORDER-RIGHT: 0px" width="60%">&nbsp;<s:property
							value="rights" /></td>
					</tr>
				</s:iterator>
		...
		<tr>
			...
			<s:if
				test="#request.roleRightBeans != null && #request.roleRightBeans.size > 0">
