...
// remove the cookie specify by name
function DeleteCookie (name) {  
  var exp = new Date();   
  exp.setTime (exp.getTime() - 1);   
  var cval = GetCookie (name);   
  document.cookie = name + "=" + cval + "; expires=" + exp.toGMTString() + ";path=/;domain=.ask.jp"; 
  // add the 'path', 'domain' if use domain, else not need to add.
} 
...
var webFXTreeConfig = {		
	rootIcon        : this.imagePath + '/o.gif',
	openRootIcon    : this.imagePath + '/o.gif',
	folderIcon      :  this.imagePath + '/ou.gif',
	openFolderIcon  :this.imagePath + '/ou.gif',
	fileIcon        : this.imagePath + '/cn.gif',
	...
	usePersistence	: true,
    disableColor : "#9B9B9B",   // [add] . 不可用时的文本颜色。
	defaultColor : "black",   //  [add] . 节点的文本颜色。
	cascadeCheck: true,    //  定义checkbox树是否级联选择。
	//imagePath : "js/images",  //  图片路径.  // 这个属性有点问题，请不要使用. 参见 setImagePath 方法.	
	setImagePath: function(path){  // 设置图片路径。直接设置 imagePath 属性不起作用。
		if(path == null) path = "pages/images/";
		if(path.charAt(path.length-1) != '/') path += "/";
        //this.imagePath = path;
		this.rootIcon = path + 'o.gif',
		this.openRootIcon = path + 'o.gif',
		this.folderIcon     =  path + 'ou.gif',
		this.openFolderIcon  = path + 'ou.gif',
		this.fileIcon       = path + 'cn.gif',
