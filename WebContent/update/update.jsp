<%@page import="org.apache.log4j.Logger"%><%@page import="java.util.List"%><%@page import="java.util.Collections"%><%@page import="java.io.*"%><%@page import="com.highguard.Wisdom.util.FileAndFolderUtil"%><%
	...
	if (request.getParameter("project_key") != null && request.getParameter("project_key").length() > 0
			&& request.getParameter("version") != null && request.getParameter("version").length() > 0) {
		String version = request.getParameter("version");
		for (int i = files.size() - 1; i >= 0; i--) {
			File f = files.get(i);
			String filename = f.getName();
			if (f.isFile() && filename.startsWith(request.getParameter("project_key"))) {
