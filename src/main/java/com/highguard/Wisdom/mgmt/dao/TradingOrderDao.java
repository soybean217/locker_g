...
public class TradingOrderDao {
		...
		if (map.get("usertype") != null && !"".equals(map.get("usertype"))) {
			...
			if ("0".equals(map.get("usertype"))) {// 主用户
				if (map.get("userid") != null && !"".equals(map.get("userid"))) {
					sql += " and (o.userid=" + map.get("userid")
							+ " and o.type='0') or  (type='1' and userid in(select id from userinfo where userId=" + map.get("userid")
							+ "))";
				}
			}
			if ("1".equals(map.get("usertype"))) {// 亲情号
				if (map.get("userid") != null && !"".equals(map.get("userid"))) {
					...
					/// sql +=" and o.id in ( select orderid from tradingorder where userid =
					/// '"+map.get("userid")+"' )";
				}
			}
		}
	...
	public void deleteOrder(int id) {
		StringBuffer hql = new StringBuffer("delete FROM orderr where id in (" + id + ")  ");
	...
	public List<TradingOrder> getTradingOrderList(Map map) {
		String sql = "select id,orderid,fruitname,latticeid,copies,price,userid,place,texture," + "CONCAT('"
				+ BasePath.baseUrl + "',imageurl) as imageurl,place " + "from tradingorder where 1=1 ";
		if (map.get("userid") != null && !"".equals(map.get("userid"))) {
	...
	public void deleteTradingOrder(int ids) {
		StringBuffer hql = new StringBuffer("delete FROM tradingorder where orderid in (" + ids + ")  ");
