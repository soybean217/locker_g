...
public class TradingApiAction extends ApiBaseAction {
	...
	public String orderList(){
		...
		Map<String,String> map = new HashMap<String, String>();
		  List<Device> device = deviceManager.getDeviceList(-1, -1, map);
		  deviceMap.put("", "-- 请选择 --");
...
//		map.put("deviceid", deciceId);
		list = tradingOrderManager.getOrderList(currentPage, pageNum, map);
		int total = tradingOrderManager.getOrderListCount(map);
...
//				list.get(i).setTradingOrderList(JSONArray.fromObject(orderList));
	...
	public void addTradingOrder(){
		...
		WisdomLogger.logInfo("addTradingOrder","orderPlaceId="+orderPlaceId+" orderinfo="+orderinfo+" userId="+userId+" usertyp="+usertype+" phone="+phone);
		...
		if(orderinfo!=null&&!"".equals(orderinfo)){
			...
			ord.setFreight("0");
			ord.setOrderPlace(orderPlace.getUsername()+" "+orderPlace.getPhone()+" "+orderPlace.getCity()+" "+orderPlace.getAddress());
			...
			for (int i = 0; i < orderstr.length; i++) {
				...
				double price = Double.parseDouble((lattice.getCopyprice()==null||"".equals(lattice.getCopyprice()))?"0":lattice.getCopyprice());//每份单价 
